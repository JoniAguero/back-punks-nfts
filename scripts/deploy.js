...
const deploy = async () => {
  ...
  // Definimos JoniPunks en el contexto
  const JoniPunks = await ethers.getContractFactory("JoniPunks");
  // Instancia del contracto desplegado
  const deployed = await JoniPunks.deploy(10000);

  console.log("JoniPunks is deployed at:", deployed.address);
};
